...
namespace DailyPlanning.Infrastructure.Database
{
    public class DailyPlanningDBInitializer : DropCreateDatabaseAlways<DailyPlanningContext>
    {
        protected override void Seed(DailyPlanningContext context)
        {
            ...
            var workitem04 = new WorkItem() { Title = "Implement string sanitization for Description in WorkItems", Description = "Because Description allow html tags to be inserted. We need a parser who is going to kick out (or replace) potentially dangerous tags (<script> ... ). Later we will make list of approved tags and use it to filter description text.", Status = Enums.Status.DONE, ProjectID = project1.ProjectID, IsEnabled = true, IsDeleted = false };
            var workitem05 = new WorkItem() { Title = "Implement Dependency Injection for context", Description = "Inject context through controller constructor.", Status = Enums.Status.DONE, ProjectID = project1.ProjectID, IsEnabled = true, IsDeleted = false };
            var workitem06 = new WorkItem() { Title = "Create validation for Add and Update", Description = "Add the same constraint to view models as they are in entities. Add name for every entity. Use DataAnnotation", Status = Enums.Status.DONE, ProjectID = project1.ProjectID, IsEnabled = true, IsDeleted = false };
            ...
            var workitem08 = new WorkItem() { Title = "Create automated ui tests for entity DailyPlan", Description = "Create coded ui tests for CRU operations regarding DailyPlan.", Status = Enums.Status.DONE, ProjectID = project2.ProjectID, IsEnabled = true, IsDeleted = false };
            ...
            var workitem10 = new WorkItem() { Title = "Add Assert in DetailsDailyPlan test method", Description = "No description provided.", Status = Enums.Status.DONE, ProjectID = project2.ProjectID, IsEnabled = true, IsDeleted = false };
            ...
            var workitem12 = new WorkItem() { Title = "Initialize database before each test scenario", Description = "Before call DropAndCreateDatabaseAlways from initializer, close all connection to the database first.", Status = Enums.Status.DONE, ProjectID = project2.ProjectID, IsEnabled = true, IsDeleted = false };
